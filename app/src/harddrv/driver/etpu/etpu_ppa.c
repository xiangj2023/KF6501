...
int32 fs_etpu_ppa_init( uint8 channel, uint8 priority, uint8 timebase,
                       uint8 mode, uint32 max_count, uint32 sample_time )

{
    uint32 *pba;	/*parameter base address for channel*/
    uint8 *pba8;	/*parameter base address for channel*/
	...
	*(pba8 + FS_ETPU_PPA_STAT_ACC_OFFSET )  =  0;
    ...
    eTPU->CHAN[channel].CR.R =
        ...
        (FS_ETPU_PPA_FUNCTION_NUMBER << 16) + (((uint32)pba -
...
uint8 fs_etpu_ppa_get_immediate( uint8 channel, uint32 *result, uint32 *current_count )
{
	uint32 *pba;	/*parameter base address for channel*/
    uint8 *pba8;
	uint8 stat_acc;
