...
******************************************************************************/
int32 fs_etpu_pwm_init( uint8 channel, uint8 priority, uint32 freq, 
      uint16 duty, uint8 polarity, uint8 timebase, uint32 timebase_freq)
{
	...
	*(pba + ((FS_ETPU_PWM_ACTIVE_OFFSET - 1)>>2))  = 
	...
	eTPU->CHAN[channel].CR.R = (priority << 28) + 
				(FS_ETPU_PWM_TABLE_SELECT << 24) +
	                        ...
	                        (((uint32)pba - fs_etpu_data_ram_start)>>3);
       ...
       uint32 timebase_freq)
{
    ...
    /* Determine frequency of output waveform */
	/*freq放大1000倍，因此需要做同步缩小处理*/
	value = timebase_freq;
	value *= 1000;
	chan_period = (uint32)((value+500)/freq);
	/*chan_period = (timebase_freq) / freq;*/
