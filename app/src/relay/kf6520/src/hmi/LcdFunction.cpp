...
extern TLoadSupV * pTodayImax;
extern TLoadSupV * pTodayUmin;
extern TLoadSupV * pYtdayImax;
extern TLoadSupV * pYtdayUmin;
extern TLoadSupV * pDaybyImax;
extern TLoadSupV * pDaybyUmin;
...
{		
	...
	WORD whAIIndex[] = {M_U1,M_U2,M_U3,M_U4,M_IT,M_IF}; 
	g_AIDlg.DoModal(this, this, hAI,6,whAIIndex,DlgCaption);
}
void CLcdWndMeasure::OnEnterBtnLMeasure()
{	
	char DlgCaption[64];
	sprintf(DlgCaption, tr("二次侧测量值\n"));	
	WORD wlAIIndex[] = {M_U1,M_U2,M_U3,M_U4,M_IT,M_IF,M_P,M_Q,M_COS,M_F};	
	g_AIDlg.DoModal(this, this, lAI,10,wlAIIndex,DlgCaption);
}
void CLcdWndMeasure::OnEnterBtnPML()
{
	...
	if(*AS_pnPM == BTMODE)
	{
		WORD wplAIIndex[] = {RM_UP,RM_U1,RM_U2,RM_U3,RM_U4,RM_IP,RM_IT,RM_IF,RM_I2,RM_I3,RM_I5,RM_R,RM_X,RM_ZA,RM_AUP,RM_AU1,RM_AU2,RM_AIP,RM_AIT,RM_AIF};	
		g_AIDlg.DoModal(this, this, plAI,20,wplAIIndex,DlgCaption);
	}
	else
	{
		WORD wplAIIndex[] = {RM_UP,RM_U1,RM_U2,RM_U3,RM_U4,RM_IP,RM_IT,RM_IF,RM_I2,RM_I3,RM_I5,RM_R,RM_X,RM_ZA,RM_TR,RM_TX,RM_TZA,RM_FR,RM_FX,RM_FZA,RM_AUP,RM_AU1,RM_AU2,RM_AIP,RM_AIT,RM_AIF,RM_AITM,RM_AIFM};	
		g_AIDlg.DoModal(this, this, plAI,28,wplAIIndex,DlgCaption);
	}
}
void CLcdWndMeasure::OnEnterBtnLHarmonic()
{
	char DlgCaption[64];
	sprintf(DlgCaption, tr("二次侧谐波值\n"));	
	WORD wHarmIndex[] = {M_I1,M_I2,M_I3,M_I5, M_I7, M_I9,M_I11};
...
void CLcdWndMeasure::OnEnterBtnLoadValue()
{
	CLcdApp::m_WndLoad.m_wDay=0;

}
BOOL CLcdWndLoad::OnWndInit()
{
	char str1[128];
	char str2[32];
	char str3[32];
	char str4[32];
	char str5[32];
	TSysTime systime1,systime2;
	if(m_wDay==0)
	{
		SetCaption(tr("负荷监测:1-3"));
		AbsTimeTo(&(pTodayImax->Time),&systime1);
		AbsTimeTo(&(pTodayUmin->Time),&systime2);
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime1.Year,systime1.Month, \
			systime1.Day,systime1.Hour,systime1.Minute,systime1.Second, \
			systime1.MSecond);
		m_StaticImaxTime.SetCaption(str1);
		sprintf(str2,"%s%d %s","Imax=",pTodayImax->Current*10/(*CF_pnIP),"A");
		m_StaticImax.SetCaption(str2);
		sprintf(str3,"%s%d.%d %s","U=",pTodayImax->Voltage/(*CF_pnUP*100),(pTodayImax->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUval.SetCaption(str3);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIM.SetCaption(str4);
		sprintf(str5,"%s%d.%d %s","∠U=",pTodayImax->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUM.SetCaption(str5);
		
		
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime2.Year,systime2.Month, \
			systime2.Day,systime2.Hour,systime2.Minute,systime2.Second, \
			systime2.MSecond);		
		m_StaticUminTime.SetCaption(str1);		
		sprintf(str3,"%s%d.%d %s","Umin=",pTodayUmin->Voltage/(*CF_pnUP*100),(pTodayUmin->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUmin.SetCaption(str3);
		sprintf(str2,"%s%d %s","I=",pTodayUmin->Current*10/(*CF_pnIP),"A");
		m_StaticIval.SetCaption(str2);		
		sprintf(str5,"%s%d.%d %s","∠U=",pTodayUmin->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUMin.SetCaption(str5);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIMin.SetCaption(str4);
	}
	else if((m_wDay == 1)||(m_wDay==-2))
	{
		SetCaption(tr("负荷监测:2-3"));
		AbsTimeTo(&(pYtdayImax->Time),&systime1);
		AbsTimeTo(&(pYtdayUmin->Time),&systime2);
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime1.Year,systime1.Month, \
			systime1.Day,systime1.Hour,systime1.Minute,systime1.Second, \
			systime1.MSecond);
		m_StaticImaxTime.SetCaption(str1);
		sprintf(str2,"%s%d %s","Imax=",pYtdayImax->Current*10/(*CF_pnIP),"A");
		m_StaticImax.SetCaption(str2);
		sprintf(str3,"%s%d.%d %s","U=",pYtdayImax->Voltage/(*CF_pnUP*100),(pYtdayImax->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUval.SetCaption(str3);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIM.SetCaption(str4);
		sprintf(str5,"%s%d.%d %s","∠U=",pYtdayImax->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUM.SetCaption(str5);
		...
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime2.Year,systime2.Month, \
			...
			systime2.MSecond);		
		m_StaticUminTime.SetCaption(str1);		
		sprintf(str3,"%s%d.%d %s","Umin=",pYtdayUmin->Voltage/(*CF_pnUP*100),(pYtdayUmin->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUmin.SetCaption(str3);
		sprintf(str2,"%s%d %s","I=",pYtdayUmin->Current*10/(*CF_pnIP),"A");
		m_StaticIval.SetCaption(str2);		
		sprintf(str5,"%s%d.%d %s","∠U=",pYtdayUmin->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUMin.SetCaption(str5);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIMin.SetCaption(str4);
	}
	else if((m_wDay ==2)||(m_wDay==-1))
	{			
		SetCaption(tr("负荷监测:3-3"));
		AbsTimeTo(&(pDaybyImax->Time),&systime1);
		AbsTimeTo(&(pDaybyUmin->Time),&systime2);
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime1.Year,systime1.Month, \
			systime1.Day,systime1.Hour,systime1.Minute,systime1.Second, \
			systime1.MSecond);
		m_StaticImaxTime.SetCaption(str1);
		sprintf(str2,"%s%d %s","Imax=",pDaybyImax->Current*10/(*CF_pnIP),"A");
		m_StaticImax.SetCaption(str2);   //注意电压系数为100V的系数
		sprintf(str3,"%s%d.%d %s","U=",pDaybyImax->Voltage/(*CF_pnUP*100),(pDaybyImax->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUval.SetCaption(str3);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIM.SetCaption(str4);
		sprintf(str5,"%s%d.%d %s","∠U=",pDaybyImax->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUM.SetCaption(str5);
		...
		sprintf(str1,"%d-%d-%d %d:%d:%d %d",systime2.Year,systime2.Month, \
			...
			systime2.MSecond);		
		m_StaticUminTime.SetCaption(str1);		
		sprintf(str3,"%s%d.%d %s","Umin=",pDaybyUmin->Voltage/(*CF_pnUP*100),(pDaybyUmin->Voltage*10/(*CF_pnUP))%1000/10,"kV");
		m_StaticUmin.SetCaption(str3);
		sprintf(str2,"%s%d %s","I=",pDaybyUmin->Current*10/(*CF_pnIP),"A");
		m_StaticIval.SetCaption(str2);
		sprintf(str5,"%s%d.%d %s","∠U=",pDaybyUmin->AngleVolt/100,pTodayImax->AngleVolt%100,"°");
		m_StaticAngleUMin.SetCaption(str5);
		sprintf(str4,"%s","∠I=0.0°");
		m_StaticAngleIMin.SetCaption(str4);
	}
}
void CLcdWndLoad::OnWndLeftRight(LONG leftRight)
{
	m_wDay += leftRight;
	m_wDay %= 3;
	OnWndInit();
...
BOOL CLcdWndSettingModify::OnWndInit()
{
	...
	char str[64];
	char strFormat[32];
	char strSetDataName[MAX_DISP_COUNT][HMI_DBSTRING_SIZE]; //kang 2012-10-31 修改数组大小
...
//kang 2012-10-31 从调用DB_GetSETDataName 改为调用TR_GetSETDataName
...
BOOL CLcdWndMainMenu::OnWndInit()
{
	...
	{		
		...
		m_BtnStatus.SetBitmap(NULL,0,0,0,0,0);
...
void CLcdWndCIPara::OnEnterBtnKWH()
{
	char Msg[64];
	BOOL IsCaluIntKW = DB_IsCaculateIntegral();
...
void CLcdWndTest::OnEnterBtnfactory()
{
	m_Btnfactory.SetEnterWnd(NULL);
