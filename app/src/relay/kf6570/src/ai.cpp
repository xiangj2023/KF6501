...
void CAIProc::Init(void)
{
	...
	m_OneKilowattP = (QWORD)*CF_pnC2*3600*1000*1000/(AI_TIMER_PERIOD*100);
	m_OneKilowattQ = (QWORD)*CF_pnC2*3600*1000*1000/(AI_TIMER_PERIOD*100);
	m_AccKilowattP = 0;
	m_AccKilowattQ = 0;
...
/*	CaluBaseFourier_S15(m_pDataBuf+READ_POINT_NUM*10,&m_sILA);
	...
	angle = m_sILA.Angle-m_UHA1.Angle;
	if(angle>0)
		*RM_pnAILA = angle;
	else
		*RM_pnAILA = angle+36000;
	...
	angle = m_sILB.Angle-m_UHA1.Angle;
	if(angle>0)
		*RM_pnAILB = angle;
	else
		*RM_pnAILB = angle+36000;
	...
	angle = m_sILC.Angle-m_UHA1.Angle;
	if(angle>0)
		*RM_pnAILC = angle;
	else
		*RM_pnAILC = angle+36000;
