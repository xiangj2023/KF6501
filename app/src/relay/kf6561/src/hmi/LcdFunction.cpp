...
{	
	...
	WORD wlAIIndex[] = {M_UA,M_UB,M_UC,M_UAB,M_UBC,M_UCA,M_U1,M_U2,M_U3,M_IA,M_IB,M_IC,M_I0,M_I1,M_P,M_Q,M_COS,M_F1};	
	g_AIDlg.DoModal(this, this, lAI,18,wlAIIndex,DlgCaption);
}
...
void CLcdWndMeasure::OnEnterBtnAngle()
{
	...
	WORD wplAIIndex[] = {RM_AUA,RM_AUB,RM_AUC,RM_AUAB,RM_AUBC,RM_AUCA,RM_AUL1,RM_AUL2,RM_AUL3, \
		RM_AIPA,RM_AIPB,RM_AIPC,RM_AIP1,RM_AIMA,RM_AIMB,RM_AIMC,RM_AU01,RM_AI01,RM_U05,RM_I05};	
...
void CLcdWndInputSetting::ModefyDIProperty()
{	
	CHmiWnd *pWnd = &(CLcdApp::m_WndInputSetting);
	STATUS rc;
	TDIPara	pDI;	
	for(LONG i=0;i<2;i++)
	{
	...
	CLcdApp::m_WndInputSetting.Show();
}
...
void CLcdWndCIPara::OnEnterBtnDIProperty()
{
		...
		if(IsPulseKW == TRUE)
		{
			...
			HMIMsgBox(tr("开入属性"),Msg,&(CLcdApp::m_Wndsyspara), this, HMI_MB_OKCANCEL,pfn);
		}
		else
		{
...
void CLcdWndCIPara::OnEnterBtnIntegralKW()
{
	...
	if(IsCaluIntKW == TRUE)
	{
		...
		HMIMsgBox(tr("积分电度"),Msg,&(CLcdApp::m_Wndsyspara), this, HMI_MB_OKCANCEL,pfn);
	}
	else
	{
