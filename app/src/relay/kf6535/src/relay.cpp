...
void CRelayTask::Init(void)
{
	...
	b68IdHMulFlag_A = FALSE;		//A相差流综合谐波闭锁标志
	b68IdHMulFlag_B = FALSE;		//B相差流综合谐波闭锁标志
	b68IdHMulFlag_C = FALSE;		//C相差流综合谐波闭锁标志

	HA_PDWithMemoryFlag = FALSE;
	HB_PDWithMemoryFlag = FALSE;
	HC_PDWithMemoryFlag = FALSE;
	
	m_bHaveTripFailedRpt_H= FALSE;	//已做断路器跳闸失败报告标志
	m_bHaveTripFailedRpt_M= FALSE;	//已做断路器跳闸失败报告标志
	m_bHaveTripFailedRpt_L = FALSE;	//已做断路器跳闸失败报告标志
	m_b52BFRecoverFLAG_H = FALSE;	//高压侧断路器跳闸失败恢复标志
	m_b52BFRecoverFLAG_M = FALSE;	//中压侧断路器跳闸失败恢复标志
	m_b52BFRecoverFLAG_L = FALSE;	//低压侧断路器跳闸失败恢复标志
	m_b52BFHFlag = FALSE;			//已检出高侧断路器跳闸失败标志
	m_b52BFMFlag = FALSE;			//已检出中侧断路器跳闸失败标志
	m_b52BFLFlag = FALSE;			//已检出低侧断路器跳闸失败标志
	...
	for(LONG i=0;i<14;i++)			//突变量启动判断两次任务间连续点计数器
	{		
		PickCheckNext[i] = 0;	
		lPickUpValueArray[i]=0;
	}
	for(LONG i=0;i<4;i++)			//数字录波组字计算结果数组
	{
		DCCaluResult[i]=0;
	}
	for(LONG i=0;i<5;i++)
	...
	dwChanDataMask = ( (0x01<<CHANNEL_6535_UA)|(0x01<<CHANNEL_6535_UB)|(0x01<<CHANNEL_6535_UC)|
						...
						(0x01<<CHANNEL_6535_U0)|(0x01<<CHANNEL_6535_I0)|(0x01<<CHANNEL_6535_Ijx));
...
//		(0x01<<CHANNEL_DC_5)|(0x01<<CHANNEL_DC_6));
	...
	dwSampBuf = (LONG*)OS_MemAlloc(sizeof(DWORD)*READ_POINT_NUM*15);	
	//合成数据回写缓冲区
	dwRecBuf = (LONG*)OS_MemAlloc(sizeof(DWORD)*RELAY_SAMP_INTERVAL*12);
	//突变量启动历史数据缓冲区
	dwPickDataBuf = (LONG*)OS_MemAlloc(sizeof(DWORD)*RELAY_SAMP_INTERVAL*3*14);
	//合成计算用数据缓冲区
	...
	memset(dwSampBuf, 0, sizeof(DWORD)*READ_POINT_NUM*15);
	...
	memset(dwPickDataBuf, 0, sizeof(DWORD)*RELAY_SAMP_INTERVAL*3*14);
...
BOOL CRelayTask::WriteComposeBuf()
{
	...
	{
		tempA1 = (CoefCorH[0]*dwSampBuf[READ_POINT_NUM*4+i]+ \
			CoefCorH[1]*dwSampBuf[READ_POINT_NUM*5+i]+CoefCorH[2]*dwSampBuf[READ_POINT_NUM*6+i])/1000;
		tempB1 = (CoefCorH[3]*dwSampBuf[READ_POINT_NUM*4+i]+ \
			CoefCorH[4]*dwSampBuf[READ_POINT_NUM*5+i]+CoefCorH[5]*dwSampBuf[READ_POINT_NUM*6+i])/1000;
		tempC1 = (CoefCorH[6]*dwSampBuf[READ_POINT_NUM*4+i]+ \
			CoefCorH[7]*dwSampBuf[READ_POINT_NUM*5+i]+CoefCorH[8]*dwSampBuf[READ_POINT_NUM*6+i])/1000;

		tempA2 = (CoefCorM[0]*dwSampBuf[READ_POINT_NUM*7+i]+ \
			CoefCorM[1]*dwSampBuf[READ_POINT_NUM*9+i]+CoefCorM[2]*dwSampBuf[READ_POINT_NUM*10+i])/1000;
		tempB2 = (CoefCorM[3]*dwSampBuf[READ_POINT_NUM*7+i]+ \
			CoefCorM[4]*dwSampBuf[READ_POINT_NUM*9+i]+CoefCorM[5]*dwSampBuf[READ_POINT_NUM*10+i])/1000;
		tempC2 = (CoefCorM[6]*dwSampBuf[READ_POINT_NUM*7+i]+ \
			CoefCorM[7]*dwSampBuf[READ_POINT_NUM*9+i]+CoefCorM[8]*dwSampBuf[READ_POINT_NUM*10+i])/1000;

		tempA3 = (CoefCorL[0]*dwSampBuf[READ_POINT_NUM*12+i]+ \
			CoefCorL[1]*dwSampBuf[READ_POINT_NUM*13+i]+CoefCorL[2]*dwSampBuf[READ_POINT_NUM*14+i])/1000;
		tempB3 = (CoefCorL[3]*dwSampBuf[READ_POINT_NUM*12+i]+ \
			CoefCorL[4]*dwSampBuf[READ_POINT_NUM*13+i]+CoefCorL[5]*dwSampBuf[READ_POINT_NUM*14+i])/1000;
		tempC3 = (CoefCorL[6]*dwSampBuf[READ_POINT_NUM*12+i]+ \
			CoefCorL[7]*dwSampBuf[READ_POINT_NUM*13+i]+CoefCorL[8]*dwSampBuf[READ_POINT_NUM*14+i])/1000;
...
{	
	...
	*RM_pnU0= U0.Mod; 									  
	...
	if(U0.Mod>(*AS_pn59UN))
		WriteRelayWord(RW_59UN, RELAY_ON);
	else
		WriteRelayWord(RW_59UN, RELAY_OFF);
	if(UC.Mod>(*AS_pn27UN))
		WriteRelayWord(RW_27UN, RELAY_OFF);
	else
		WriteRelayWord(RW_27UN, RELAY_ON);
	//IHA
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*4,&IHA);
	CaluModValue(&IHA); 
	*RM_pnIHA= IHA.Mod; 									  
	...
	//IHB
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*5,&IHB);
	CaluModValue(&IHB); 
	*RM_pnIHB= IHB.Mod; 									  
	...
	//IHC
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*6,&IHC);
	CaluModValue(&IHC); 
	*RM_pnIHC= IHC.Mod; 									  
	...
	//IMA
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*7,&IMA);
	CaluModValue(&IMA); 
	*RM_pnIMA= IMA.Mod; 									  
	...
	//IMB
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*9,&IMB);
	CaluModValue(&IMB); 
	*RM_pnIMB= IMB.Mod; 									  
	...
	//IMC
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*10,&IMC);
	CaluModValue(&IMC); 
	*RM_pnIMC= IMC.Mod; 									  
	...
	//ILA
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*12,&ILA);
	CaluModValue(&ILA); 
	*RM_pnILA= ILA.Mod; 									  
	...
	//ILB
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*13,&ILB);
	CaluModValue(&ILB); 
	*RM_pnILB= ILB.Mod; 									  
	...
	//ILC
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*14,&ILC);
	CaluModValue(&ILC); 
	*RM_pnILC= ILC.Mod; 									  
	...
	//I0
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*8,&I0);
	CaluModValue(&I0); 
	*RM_pnI0= I0.Mod; 									  
	DB_LogWriteRM(RM_I0, *RM_pnI0);
	if(I0.Mod>(*AS_pn50INC))
		WriteRelayWord(RW_50IN, RELAY_ON);
	else
		WriteRelayWord(RW_50IN, RELAY_OFF);
	if(I0.Mod>(*AS_pn37INC))
		WriteRelayWord(RW_37IN, RELAY_OFF);
	else
		WriteRelayWord(RW_37IN, RELAY_ON);
	//ILC
	CaluBaseFourier_S15(dwSampBuf+READ_POINT_NUM*11,&IJX);
	CaluModValue(&IJX); 
	*RM_pnIjx= IJX.Mod; 									  
	DB_LogWriteRM(RM_IJX, *RM_pnIjx);
	if(IJX.Mod>(*AS_pn50IJXC))
...
*备注:
******************************************************************************************/
void CRelayTask::InitR59UN()
{	
	R59U0.pElecSet			= RS_pn59UNV;          			//过量保护电量定值
	R59U0.pTimeSet			= RS_pn59UND;				 	//过量保护时间定值
	R59U0.pHarmKhr1Set		= NULL;							//过量保护谐波加权系数
	R59U0.pVolLockSet			= NULL;				 			//过量保护电压闭锁定值
	R59U0.pAccActTimeSet		= NULL;							//过量保护后加速动作时间定值
	R59U0.pH2Coef				= NULL;							//二次谐波闭锁系数，谐波加权系数的放大倍数
	
	R59U0.pRelCFG				= CFG_pby59UN;				//过量保护功能控制字--1:投入，2:退出 
	R59U0.pRelSW				= SW_pby59UN;         			//过量保护软压板------1:投入，2:退出
	R59U0.pRelHLockCFG		= &RelCFGCLR;			    	//谐波闭锁功能控制位--1:投入，2:退出
	R59U0.pRelHSuprCFG		= &RelCFGCLR; 					//谐波抑制功能控制位--1:投入，2:退出
	R59U0.pRelPDCFG			= &RelCFGCLR;		    		//功率方向功能控制位--1:投入，2:退出
	R59U0.pRelVolLockCFG		= &RelCFGCLR;					//低压闭锁功能控制位--1:投入，2:退出
	R59U0.pRelFDCFG			= &RelCFGCLR;					//功率方向功能控制位--1:正方向，2:反方向
	R59U0.pRelAccCFG			= &RelCFGCLR;					//有无后加速功能------1:有，  2:无
	R59U0.pRelFLCFG			= &RelCFGCLR;					//故障测距投入
	R59U0.pSysTimer			= &dwRelayMSCnt;				//系统定时器指针	
	
	R59U0.pRelElec 			= &U0;							//当前电量指针		
	R59U0.pRelHarm3			= NULL;							//当前三次谐波指针	
	R59U0.pRelHarm5			= NULL;							//当前五次谐波指针	
	R59U0.pRelVolt			= NULL;							//当前用来闭锁的电压的指针
	R59U0.pRel47UCFG		= &RelCFGCLR;;
	R59U0.R47ULKRelayWord   = FALSE1;

	R59U0.StartRelayWord		= RW_PI59UN;					//过量保护入段继电器字
	R59U0.ActRelayWord		= RW_TR59UN;					//过量保护动作继电器字
	R59U0.AccRelayWord		= FALSE1;						//后加速允许继电器字
	R59U0.AccActRelayWord		= FALSE1;						//后加速动作继电器字
	R59U0.PForwardRelayWord	= FALSE1;						//功率正方向继电器字
	R59U0.PRewardRelayWord   	= FALSE1;						//功率反方向继电器字
	R59U0.H2LockRelayWord		= FALSE1;						//二次谐波闭锁继电器字
	R59U0.LockRelayWord		= RW_LKPI59UN;					//入段闭锁继电器字
	R59U0.RSTRelayWord		= RW_LKRST59UN;					//返回闭锁继电器字
	R59U0.LoopCtrlRelayWord	= RW_LPC59UN;					//保护循环控制继电器字
	
	R59U0.wReportNum			= R_R59UN;						//过量保护报告序号
	R59U0.wAccReportNum		= 0xFFFF;						//加速动作报告序号
	R59U0.bMakeActRpt			= TRUE;
	R59U0.dwK35				= NULL;
	R59U0.RptSerialNum		= &RptSerialNumOfAct;	
	//**********************入段返回报告部分*********************	yanxs  13-03-21
	R59U0.wPickUpReportNum	= R_R59UN_PI;						//入段报告序列号
	R59U0.wResetReportNum		= R_R59UN_RES;						//返回报告序列号
	//**********************************************************************	
//	R59U0.dwRelTimer			= 0;			 			//定时器
	R59U0.byRelFCounter		= 0;						//故障计数器
	R59U0.byRelRCounter		= 0;						//返回计数器
	
	R59U0.wExcepWordNum     = 8;
}
//******************************************************************************************
// 名称: 初始化零序过流保护元件                               
// 输入:                                          
...
// 返回: 无                                                                                                                                                               
//******************************************************************************************
void CRelayTask::InitR50IN()
{	
	R50I0.pElecSet			= RS_pn50INC;          			//过量保护电量定值
	R50I0.pTimeSet			= RS_pn50IND;				 	//过量保护时间定值
	...
	R50I0.pRelCFG				= CFG_pby50IN;				//过量保护功能控制字--1:投入，2:退出 
	R50I0.pRelSW				= SW_pby50IN;         			//过量保护软压板------1:投入，2:退出
	...
	R50I0.pRelFDCFG			= &RelCFGCLR;					//功率方向功能控制位--1:正方向，2:反方向
	R50I0.pRelAccCFG			= &RelCFGCLR;					//有无后加速功能------1:有，  2:无
	R50I0.pRelFLCFG			= &RelCFGCLR;					//故障测距投入
	R50I0.pSysTimer			= &dwRelayMSCnt;				//系统定时器指针	
	
	R50I0.pRelElec 			= &I0;							//当前电量指针		
	R50I0.pRelHarm3			= NULL;							//当前三次谐波指针	
	R50I0.pRelHarm5			= NULL;							//当前五次谐波指针	
	R50I0.pRelVolt			= NULL;							//当前用来闭锁的电压的指针
	R50I0.pRel47UCFG		= &RelCFGCLR;;
	R50I0.R47ULKRelayWord   = FALSE1;

	R50I0.StartRelayWord		= RW_PI50IN;					//过量保护入段继电器字
	R50I0.ActRelayWord		= RW_TR50IN;					//过量保护动作继电器字
	R50I0.AccRelayWord		= FALSE1;						//后加速允许继电器字
	R50I0.AccActRelayWord		= FALSE1;						//后加速动作继电器字
	R50I0.PForwardRelayWord	= FALSE1;						//功率正方向继电器字
	R50I0.PRewardRelayWord   	= FALSE1;						//功率反方向继电器字
	R50I0.H2LockRelayWord		= FALSE1;						//二次谐波闭锁继电器字
	R50I0.LockRelayWord		= RW_LKPI50IN;					//入段闭锁继电器字
	R50I0.RSTRelayWord		= RW_LKRST50IN;					//返回闭锁继电器字
	R50I0.LoopCtrlRelayWord	= RW_LPC50IN;					//保护循环控制继电器字
	
	R50I0.wReportNum			= R_R50IN;						//过量保护报告序号
	R50I0.wAccReportNum		= 0xFFFF;						//加速动作报告序号
	R50I0.bMakeActRpt			= TRUE;
	R50I0.dwK35				= NULL;
	R50I0.RptSerialNum		= &RptSerialNumOfAct;	
	//**********************入段返回报告部分*********************	yanxs  13-03-21
	R50I0.wPickUpReportNum	= R_R50IN_PI;						//入段报告序列号
	R50I0.wResetReportNum		= R_R50IN_RES;						//返回报告序列号
	//**********************************************************************	
...
// 返回: 无                                                                                                                                                               
//------------------------------------------------------------------------------------------ 
void CRelayTask::InitR50I_JX()
{	
	R50IJX.pElec1Set			= RS_pn50IJXC;          			//过量保护电量定值	
	R50IJX.pElec2Set			= RS_pn59UNV;          			//过量保护电量定值
	R50IJX.pTimeSet			= RS_pn50IJXD;				 	//过量保护时间定值
	R50IJX.pHarmKhr1Set		= NULL;							//过量保护谐波加权系数
	R50IJX.pVolLockSet		= NULL;				 	//过量保护电压闭锁定值
	R50IJX.pAccActTimeSet	= NULL;							//过量保护后加速动作时间定值
	R50IJX.pH2Coef			= NULL;							//二次谐波闭锁系数，谐波加权系数的放大倍数
	
	R50IJX.pRelCFG			= CFG_pby50IJX;				//过量保护功能控制字--1:投入，2:退出 
	R50IJX.pRelSW			= SW_pby50IJX;         			//过量保护软压板------1:投入，2:退出
	R50IJX.pRelHLockCFG		= &RelCFGCLR;			    	//谐波闭锁功能控制位--1:投入，2:退出
	R50IJX.pRelHSuprCFG		= &RelCFGCLR; 					//谐波抑制功能控制位--1:投入，2:退出
	R50IJX.pRelPDCFG			= &RelCFGCLR;		    		//功率方向功能控制位--1:投入，2:退出
	R50IJX.pRelVolLockCFG	= &RelCFGCLR;				//低压闭锁功能控制位--1:投入，2:退出
	R50IJX.pRelFDCFG			= &RelCFGCLR;					//功率方向功能控制位--1:正方向，2:反方向
	R50IJX.pRelAccCFG		= &RelCFGCLR;					//有无后加速功能------1:有，  2:无
	R50IJX.pRelFLCFG			= &RelCFGCLR;					//故障测距投入
	R50IJX.pSysTimer			= &dwRelayMSCnt;				//系统定时器指针	
	
	R50IJX.pRelElec1 		= &IJX;							//当前电量指针	
	R50IJX.pRelElec2 		= &U0;							//当前电量指针	
	R50IJX.pRelHarm3			= NULL;							//当前三次谐波指针	
	R50IJX.pRelHarm5			= NULL;							//当前五次谐波指针	
	R50IJX.pRelVolt			= NULL;							//当前用来闭锁的电压的指针
	R50IJX.pRel47UCFG		= &RelCFGCLR;;
	R50IJX.R47ULKRelayWord   = FALSE1;

	R50IJX.StartRelayWord	= RW_PI50IJX;					//过量保护入段继电器字
	R50IJX.ActRelayWord		= RW_TR50IJX;					//过量保护动作继电器字
	R50IJX.AccRelayWord		= FALSE1;						//后加速允许继电器字
	R50IJX.AccActRelayWord	= FALSE1;						//后加速动作继电器字
	R50IJX.PForwardRelayWord	= FALSE1;						//功率正方向继电器字
	R50IJX.PRewardRelayWord  = FALSE1;						//功率反方向继电器字
	R50IJX.H2LockRelayWord	= FALSE1;						//二次谐波闭锁继电器字
	R50IJX.LockRelayWord		= RW_LKPI50IJX;					//入段闭锁继电器字
	R50IJX.RSTRelayWord		= RW_LKRST50IJX;				//返回闭锁继电器字
	R50IJX.LoopCtrlRelayWord	= RW_LPC50IJX;					//保护循环控制继电器字
	
	R50IJX.wReportNum		= R_R50IJX;						//过量保护报告序号
	R50IJX.wAccReportNum		= 0xFFFF;						//加速动作报告序号

	R50IJX.bMakeActRpt		= TRUE;
	R50IJX.dwK35				= NULL;
	R50IJX.RptSerialNum		= &RptSerialNumOfAct;						
	//**********************入段返回报告部分*********************	yanxs  13-03-21
	R50IJX.wPickUpReportNum	= R_R50IJX_PI;						//入段报告序列号
	R50IJX.wResetReportNum	= R_R50IJX_RES;						//返回报告序列号
	//**********************************************************************	

//	R50IJX.dwRelTimer		= 0;			 			//定时器
	R50IJX.byRelFCounter		= 0;						//故障计数器
	R50IJX.byRelRCounter		= 0;						//返回计数器
	
	R50IJX.wExcepWordNum     = 8;
}
...
void CRelayTask::InitStructCheck()
{
	...
	StructCheck[16].pstruct = (WORD*)&R59U0;
	StructCheck[16].bigsize = sizeof(R59U0)/2 - R59U0.wExcepWordNum;	
	StructCheck[17].pstruct = (WORD*)&R50I0;
	StructCheck[17].bigsize = sizeof(R50I0)/2 - R50I0.wExcepWordNum;	
	StructCheck[18].pstruct = (WORD*)&R50IJX;
	StructCheck[18].bigsize = sizeof(R50IJX)/2 - R50IJX.wExcepWordNum;	
	StructCheck[19].pstruct = (WORD*)&R87Alarm;
	StructCheck[19].bigsize = sizeof(R87Alarm)/2 - R87Alarm.wExcepWordNum;	
	StructCheck[20].pstruct = (WORD*)SW_BUF_ADDR;
	StructCheck[20].bigsize = (CF_BUF_ADDR+CF_BUF_SIZE-SW_BUF_ADDR)/2;
...
{			
	...
	if((*R59U0.pRelCFG == CFG_STATUS_SET) && (*R59U0.pRelSW == SW_STATUS_SET))   //零压
	{
		relayInputFlag = TRUE;
		if(R59U0.pRelElec->Mod > *R59U0.pElecSet)
		{
			WriteRelayWord(PickUpRW, RELAY_ON);
			ReadAbsTime(&PickUpTime);
			DB_RelayStart(++g_RptSerialNum,ReportInf[R_ACTPICK].wInf,&PickUpTime,FALSE); //向数据库中写入启动报告
	...
	if((*R50I0.pRelCFG == CFG_STATUS_SET) && (*R50I0.pRelSW == SW_STATUS_SET))   //零流
	{
		relayInputFlag = TRUE;
	...
	if((*R50IJX.pRelCFG == CFG_STATUS_SET) && (*R50IJX.pRelSW == SW_STATUS_SET))   //间隙过流
	{
		relayInputFlag = TRUE;
		if(R50IJX.pRelElec1->Mod > *R50IJX.pElec1Set)
		{
			WriteRelayWord(PickUpRW, RELAY_ON);
			ReadAbsTime(&PickUpTime);
			DB_RelayStart(++g_RptSerialNum,ReportInf[R_ACTPICK].wInf,&PickUpTime,FALSE); //向数据库中写入启动报告
	...
	{		
		BYTE  pChannel[]={CHANNEL_6535_IHA,CHANNEL_6535_IHB,CHANNEL_6535_IHC,
							CHANNEL_6535_IMA,CHANNEL_6535_IMB,CHANNEL_6535_IMC,
							CHANNEL_6535_ILA,CHANNEL_6535_ILB,CHANNEL_6535_ILC,
							CHANNEL_6535_IDA,CHANNEL_6535_IDB,CHANNEL_6535_IDC,
							CHANNEL_6535_I0,CHANNEL_6535_Ijx};
		STATUS rc;
		rc = ReadPreSingleData(dwPickDataBuf,pChannel,14,RELAY_SAMP_INTERVAL);
		...
		for(LONG j=0;j<14;j++)//根据需突变启动的通道数
		{
			PickCheck = 0;
			for(LONG i=0;i<RELAY_SAMP_INTERVAL;i++)
			{			
...
// 返回: 无                                                                                   
//------------------------------------------------------------------------------------------ 
void CRelayTask::FaultProcess(void)
{ 	
...
//			return;							//北京型式实验加速保护动作时间封掉
	...
	else
	{
		...
		GeneralOverValueRelay(&R59U0);				//零序过压
		GeneralOverValueRelay(&R50I0);				//零序过流
		GeneralOverValueRelay_3(&R50IJX);			//间隙过流
		
		if(ReadRelayWord(RW_52BFH))
		{
			if((!m_bHaveTripFailedRpt_H)&&(!m_b52BFRecoverFLAG_H))
			{
				m_b52BFRecoverFLAG_H = TRUE;
				MakeActRpt(R_TRFAIL_H,RptSerialNumOfAct);		//作高断路器跳闸失败报告
				m_bHaveTripFailedRpt_H= TRUE;
			}
		}
		if(ReadRelayWord(RW_52BFM))
		{
			if((!m_bHaveTripFailedRpt_M)&&(!m_b52BFRecoverFLAG_M))
			{
				m_b52BFRecoverFLAG_M = TRUE;
				MakeActRpt(R_TRFAIL_M,RptSerialNumOfAct);		//作中断路器跳闸失败报告
				m_bHaveTripFailedRpt_M = TRUE;
			}
		}
		if(ReadRelayWord(RW_52BFL))
		{
			if((!m_bHaveTripFailedRpt_L)&&(!m_b52BFRecoverFLAG_L))
			{
				m_b52BFRecoverFLAG_L = TRUE;
				MakeActRpt(R_TRFAIL_L,RptSerialNumOfAct);		//作低断路器跳闸失败报告
				m_bHaveTripFailedRpt_L = TRUE;
			}
		}
		...
		if(ReadRelayWord(R87U_A.LoopCtrlRelayWord)||ReadRelayWord(R87U_B.LoopCtrlRelayWord)||ReadRelayWord(R87U_C.LoopCtrlRelayWord) \
		   ...
		   ||((!ReadRelayWord(RW_52BFM))&&(ReadRelayWord(R50IM1.LoopCtrlRelayWord)||ReadRelayWord(R50IM2.LoopCtrlRelayWord))) \
		   ||((!ReadRelayWord(RW_52BFL))&&(ReadRelayWord(R50IL1.LoopCtrlRelayWord)||ReadRelayWord(R50IL2.LoopCtrlRelayWord))) \
		   ||ReadRelayWord(R50IH1.LoopCtrlRelayWord)||ReadRelayWord(R50IH2.LoopCtrlRelayWord)||ReadRelayWord(R59U0.LoopCtrlRelayWord)\
		   ||ReadRelayWord(R50I0.LoopCtrlRelayWord)||ReadRelayWord(R50IJX.LoopCtrlRelayWord))
		{
			if(ReadRelayWord(RW_52BFH))
			{				
			...
			if((ReadRelayWord(RW_52BFM))||(ReadRelayWord(RW_52BFL)))
			{				
				if(ReadRelayWord(R87U_A.LoopCtrlRelayWord)||ReadRelayWord(R87U_B.LoopCtrlRelayWord)||ReadRelayWord(R87U_C.LoopCtrlRelayWord) \
					||ReadRelayWord(R87R_A.LoopCtrlRelayWord)||ReadRelayWord(R87R_B.LoopCtrlRelayWord)||ReadRelayWord(R87R_C.LoopCtrlRelayWord) \
					||ReadRelayWord(R50IH1.LoopCtrlRelayWord)||ReadRelayWord(R50IH2.LoopCtrlRelayWord)||ReadRelayWord(R59U0.LoopCtrlRelayWord) \
				...
				else
				{
					...
					m_bHaveTripFailedRpt_M = FALSE; 			//已做断路器跳闸失败报告标志
					m_bHaveTripFailedRpt_L = FALSE; 			//已做断路器跳闸失败报告标志
					HA_PDWithMemoryFlag = FALSE;				//清带记忆功率方向判断标志
					HB_PDWithMemoryFlag = FALSE;
					HC_PDWithMemoryFlag = FALSE;
					return; 				
				}
		...
		else				
		{
			...
			{
				if((ReadRelayWord(RW_T52BFH))||(ReadRelayWord(RW_T52BFM))||(ReadRelayWord(RW_T52BFL)))
				{
					FaultLockTime = dwRelayMSCnt;
					return;
				}
...
void CRelayTask::Process52BF()
{
	...
	if(m_b52BFMFlag)
	{
		if(!ReadRelayWord(RW_52BFM))
		{			
			m_b52BFRecoverFLAG_M = FALSE;
			m_b52BFMFlag = FALSE;
			ResetOverValueRelay(&R50IM1,TRUE);
			ResetOverValueRelay(&R50IM2,TRUE);
		}
	}
	...
	if(m_b52BFLFlag)
	{
		if(!ReadRelayWord(RW_52BFL))
		{
			m_b52BFRecoverFLAG_L = FALSE;
			m_b52BFLFlag = FALSE;
			ResetOverValueRelay(&R50IL1,TRUE);
			ResetOverValueRelay(&R50IL2,TRUE);
		}
	}
}
//------------------------------------------------------------------------------------------
...
{			
	...
	ResetOverValueRelay(&R59U0,resetFlag);
	ResetOverValueRelay(&R50I0,resetFlag);
	ResetOverValueRelay_3(&R50IJX,resetFlag);
