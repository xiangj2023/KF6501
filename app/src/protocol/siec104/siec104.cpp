...
BEGIN_MESSAGE_MAP(CSiec104,CProtocol)
	ON_SM_COMMIDLE(CSiec104)
	ON_SM_TIMEOUT(CSiec104)
	ON_SM_SOE(CSiec104)
	ON_SM_BIBURST(CSiec104)
	ON_SM_AIE(CSiec104)
	ON_SM_DIST(CSiec104)
	ON_SM_RELAYEVENT(CSiec104)
	ON_SM_BOSELECTECHO(CSiec104)
	ON_SM_BOEXECECHO(CSiec104)
	ON_SM_NOTIFYETHLINKED(CSiec104)  //以太网建立连接
	ON_SM_NOTIFYETHCLOSED(CSiec104)  //以太网关闭连接
END_MESSAGE_MAP()
...
{
	CSiec104 *pS104=new CSiec104(pdwApp);
	if(pS104->Init()!=TRUE)
		pS104->Exit((char *)"S104 Init Error");
    	pS104->Run();
}

BOOL CSiec104::HaveNewReport(void)
{
	WORD wNum;
...
// 从未读取的报告中选择最老的一条
BOOL CSiec104::SearchNewReport(TRelayReport &Report,BYTE &Type)
{
	...
	if((DB_GetRelayAlarmReportNum(&wNum) == DB_ERR_OK )&&(wNum > m_nALMNoAckCnt))
	{
		...
		{
			LogError("CSiec104::SearchNewReport",FILE_LINE,"Read alarm report error!");
	...
	if((DB_GetRelayActionReportNum(&wNum) == DB_ERR_OK )&&(wNum > m_nACTNoAckCnt))
	{
		...
		{
			LogError("CSiec104::SearchNewReport",FILE_LINE,"Read action report error!");
...
BOOL CSiec104::SearchChangedAI(WORD *pAINo,long *pAIValue, DWORD ChangedValue, WORD wControlWord)//查找变化遥测
{
	long 	NewAIValue;
	long 	OldAIValue;
...
BOOL CSiec104::MySetTimer(BYTE bTimerID, WORD wSecond)
{
...
BOOL CSiec104::MyKillTimer(BYTE bTimerID)
{
...
void CSiec104::MyOnTimer(BYTE bTimerID)
{	
...
void CSiec104::RetryFrame(BYTE bItem)  
{                                       
...
BOOL CSiec104::KillPrevFrame(WORD dwTxdNum)
{       
	int 	i,j;
	BYTE	NoAckNum,AckNo;
...
BOOL CSiec104::SetBO(TBO* pBO,BYTE NeedReturnFlag)//遥控设置函数
{
	BYTE Mode;
	WORD SwitchNo;
...
BOOL CSiec104::SetBO(WORD BONo, WORD Command,WORD Attrib,BYTE NeedReturnFlag)//遥控设置函数
{
	TBO BO;
...
BYTE CSiec104::CheckSum(BYTE * pBuf,DWORD dwLength)
{
	BYTE bySum=0;
...
BOOL CSiec104::DistFileName2Time(char *strFileName,TSysTime *pTime)
{
	if(pTime == NULL)
		return FALSE;
...
// 初始化录波文件信息
BOOL CSiec104::InitDistFile(BYTE nFileName)
{
	...
	if( m_DistFile.pFileBuf == NULL )
	{
		ReportMsg("CSiec104::IinitDistFile() File %s buffer malloc error!",strFileName);
		return FALSE;
	}
...
BOOL CSiec104::SelectFile(BYTE nFileName)
{
...
BOOL CSiec104::SelectSec(BYTE nSecName)
{
...
void CSiec104::OnCommIdle(DWORD dwLen,DWORD PortID)
{
	TxdMonitor();
}

void CSiec104::OnTimeOut(DWORD dwTimerID)
{
	//定时器计数	
	m_dwTaskRunCount++;
...
{	
	if(GetNewAIENum() > 0)
	{
//		ReportMsg("S104:	On AIE! Num=%d",GetNewAIENum());
		SetFlag(SF_HAVEAIE);
	}
}

void CSiec104::OnRelayEvent(DWORD MsgCode, DWORD wParam)
{	
	if(HaveNewReport())
		SetFlag(SF_HAVEREPORT);
}
...
//遥控预置返校消息映射
void CSiec104::OnBOSelectEcho(WORD switchno,WORD attrib,WORD result)	
{
	m_BO.RetResult = result;
	Txd68_2E2F(7);
}

void CSiec104::OnBOExecEcho(WORD switchno,WORD attrib,WORD result)	
{	
	m_BO.RetResult = result;
	Txd68_2E2F(7);
...
void CSiec104::OnEthLinked(DWORD dwPara)
{
	ResetVar();
#ifdef S104_DEBUG
	LogError("OnEthLinked",FILE_LINE,"Link accept! Port=%x",m_wPortID);
#endif
}

void CSiec104::OnEthClosed(DWORD dwPara)
{
	ResetVar();
	RegisterCommStatus(COMM_STATUS_BREAK);
#ifdef S104_DEBUG
	LogError("OnEthClosed",FILE_LINE,"Link break! Port=%x",m_wPortID);
#endif
}

void CSiec104::OnDisturb(DWORD FileNo)
{
...
//  函数功能: 接收处理
//==============================================
void CSiec104::OnFrame(BYTE* pBuf)
{
	//处理通信状态
...
//      的字节,从而加快搜索速度
//=========================================================================
DWORD CSiec104::SearchOneFrame(PBYTE Buf,WORD Len)
{
...
BOOL CSiec104::Handle_I_Format() //处理I格式
{  
	WORD dwAckCount=0;                                   
...
BOOL CSiec104::Handle_U_Format() //处理U格式
{
	BYTE Control = 0;
...
BOOL CSiec104::Handle_S_Format() //处理S格式
{
	WORD dwAckCount;
...
BOOL CSiec104::Rxd68(void)
{
	switch(pRxdFm->Style) //类型标志符
	{
		case 0x2D:  break;//45 单命令
		case 0x2E:  //46 双命令
		case 0x2F:  Rxd68_2E2F();  break;//遥控,101规约ASDU  升降命令
		...
		case 147:	Rxd_147(); break;	//修改定值区预置 、修改定值区执行
...
BOOL CSiec104::Rxd68_68(void)//测试链路
{
	return Txd68_68();
}

BOOL CSiec104::Rxd68_6B(void)//带时标测试链路
{
	return Txd68_6B();
}

BOOL CSiec104::Rxd68_6E(void)//设置参数
{
	return Txd68_6E();
}

BOOL CSiec104::Rxd68_65_5(void)//召唤某组电度
{
	return Txd68_65_5();
}

BOOL CSiec104::Rxd68_65_6(void)//召唤所有电度
{
    WORD FlagNo;
...
BOOL CSiec104::Rxd68_65(void)//召唤电度
{
...
BOOL CSiec104::Rxd68_67(void)//时钟同步
{
...
BOOL CSiec104::Rxd68_64(void)//总召唤/召唤某一组数据
{
	WORD  GroupNo;
...
BOOL CSiec104::Rxd68_64_14(void)//处理总召唤命令
{
...
BOOL CSiec104::Rxd68_64_14_6(void)//处理总召唤激活命令
{
...
BOOL CSiec104::Rxd68_66(void)//读数据处理
{
   PBYTE pData;
...
BOOL CSiec104::Rxd68_2E2F(void)//遥控处理
{  
	switch(pRxdFm->Reason_L)//传送原因
	{
		case 6:	Rxd68_2E2F_6(); return TRUE;//遥控预置/执行处理
		case 8: Rxd68_2E2F_8(); return TRUE;//停止激活
	}
	return TRUE;
}

BOOL CSiec104::Rxd68_2E2F_6(void)//遥控预置/执行命令
{
	PBYTE 	pData=&pRxdFm->Data;
	WORD  	DCO;  //遥控命令限定词
	WORD  	BONo; //遥控路号
	...
	case 0x2F:  BONo-=AO_ADDR_START;  Is101BO = FALSE;break;
	}
	...
	case 0:	//执行
		Command=PBO_EXECUTE; 
		LogError("Rxd68_2E2F_6",FILE_LINE,"EXUTE Command = %d  No= %d  Attrib= %d\n",Command,BO.No,BO.Attrib);
		break;

	case 0x80:	
		Command=PBO_SELECT;
		LogError("Rxd68_2E2F_6",FILE_LINE,"SELECT Command = %d  No= %d  Attrib= %d\n",Command,BO.No,BO.Attrib);
		break;
	}
...
BOOL CSiec104::Rxd68_2E2F_8(void)//遥控撤销命令
{   
	SetBO(PBO_CANCEL);

	SetFlag(SF_BOEND);

	return Txd68_2E2F(9);//确认
}

//104规约带时标遥控的处理
/*
BOOL CSiec104::Rxd68_3B3C(void)//带时标遥控处理
{  
	switch(pRxdFm->Reason_L)//传送原因
	{
	case 6:	Rxd68_3B3C_6(); return TRUE;//遥控预置/执行处理
	case 8: Rxd68_3B3C_8(); return TRUE;//停止激活
	}
	return TRUE;
}

BOOL CSiec104::Rxd68_3B3C_6(void)//带时标遥控预置/执行命令
{
	PBYTE pData=&pRxdFm->Data;
	WORD  DCO;  //遥控命令限定词
	WORD  BONo; //遥控路号
	...
	switch(pRxdFm->Style)
	{
	case 0x2E:  BONo-=BO_ADDR_START;  break;
	case 0x2F:  BONo-=AO_ADDR_START;  break;
	}
...
BOOL CSiec104::Rxd68_3B3C_8(void)//遥控撤销命令
{
	SetBO(PBO_CANCEL);
	return Txd68_3B3C(9);
}
...
// 函数改写：
//=======================================================================================
BOOL CSiec104::Rxd_145(void)
{
	PBYTE 			pData=&pRxdFm->Data;
	TGenData		*pGenData;
	BYTE 			bINF = pData[3];

	if(bINF != INF_M_GEN_RGROUPVALUE)	// 241   //读一个组的全部条目的值或属性
		return FALSE;

	//通用数据区指针
	pGenData = (TGenData *)&pData[5];
	BYTE  group = pGenData->Gin.byGroup;

	if(group == 0)
	{
		if(pGenData->Gin.byEntry == 1)
			Txd68_146_STSReadEcho();   //取定值区号应答 
	}
	else if(group > 0 && pGenData->Gin.byEntry == 0)
		Rxd68_145_STRead();   //取定值应答

	return TRUE;
}

//147 103的通用分类命令 type:10
//修改定值区预置 、修改定值区执行
//=======================================================================================
// 函数名称: Rxd_147
// 函数参数：void 
// 函数返回：bool 处理正确？1是，0不是
// 函数功能: 修改定值区预置 、修改定值区执行,147: 103的通用分类命令 type:10
// 函数编写：lz 2011-11-26
// 函数改写：
//=======================================================================================
BOOL CSiec104::Rxd_147(void)
{
	PBYTE 			pData=&pRxdFm->Data;
	TGenData		*pGenData;
	BYTE 			bINF = pData[3];
	...
	switch(bINF)
	{
		case INF_M_GEN_WENTRYACK:		//249   带确认的写条目
			...
			if( pGenData->Gin.byGroup == 00  && pGenData->Gin.byEntry == 0x01)			 
				Txd68_148_STSSelectEcho();	//修改定值区号预置 
			else 
				Rxd68_147_STSelect();		//修改定值and软压板预置 
			break;

		case INF_M_GEN_WENTRYEXEC:		//250   带执行的写条目
		case INF_M_GEN_WENTRYCANCEL:	//251	写条目终止
			if( pGenData->Gin.byGroup == 00  && pGenData->Gin.byEntry == 0x01)			 
				Txd68_148_STSExecEcho();	//修改（定值区号）执行/撤销   
			else
				Txd68_148_STExecEcho();    	//修改定值执行/撤销
			break;
		default:
			break;
	}
...
// 函数改写：
//=======================================================================================
BOOL CSiec104::Rxd68_145_STRead(void)
{
	PBYTE 		pData=&pRxdFm->Data;
	TGenData	*pGenData = (TGenData *)&pData[5];
	BYTE		num,status;
	STATUS	 	rc=DB_ERR_OK;
	float		*pfValue;
	WORD		wSWValue = 0;
	...
	do{		
		...
		for(int i=0;(i<num)&&(i<16);i++)
		{
			...
			if(status == SW_STATUS_SET)
				wSWValue |= (BYTE)1<<(i%16);
			else
				wSWValue &= ~((BYTE)1<<(i%16));
		}

		*pfValue = (float)wSWValue;
		...
		m_ST.Offset = 0;
		m_ST.Num = m_LocalSTNum+1;
		
		rc = DB_GetSETFloatValue(m_ST.Group, m_ST.No,pfValue+1);
		if(rc != DB_ERR_OK)
		{
			m_ST.Result = 1;
			break;
...
// 函数改写：
//=======================================================================================
BOOL CSiec104::Rxd68_147_STSelect(void)
{
	PBYTE		pData=&pRxdFm->Data;
	TGenData	*pGenData = (TGenData *)&pData[5];
	...
	if(pGenData->Gin.byEntry == 0)
	{
		...
		m_ST.Num = m_LocalSTNum+1;//加上一个软压板
		m_ST.Offset = 0;
	}
	...
	if(pGenData->Ngd.Cont == 0)
	{
		...
		wSWValue = (WORD)(*pfValue);
		wSWValue = MAKEWORD(HIBYTE(wSWValue), LOBYTE(wSWValue));
		rc = DB_SetAllSWStatus((BYTE *)&wSWValue);
		if(rc != DB_ERR_OK)
		{
			m_ST.Result = rc;
		...
		// 再写入定值
		MergeConfigWord(pfValue+1);
		rc = DB_SetSETValue(m_ST.Group,SET_VALUE_ALL,pfValue+1);
		if(rc != DB_ERR_OK)
		{
			m_ST.Result = rc;
...
BOOL CSiec104::Rxd_124(void)	//确认文件
{
	PBYTE 	pData=&pRxdFm->Data;
	WORD	nFileName;
	BYTE	nSecName;
...
//处理总召唤电度
BOOL CSiec104::TxdCallAllCI(void)
{
...
BOOL CSiec104::Txd68_68(void)//测试链路
{
...
BOOL CSiec104::Txd68_6B(void)//测试链路
{
...
BOOL CSiec104::Txd68_6E(void)//设置参数
{
...
BOOL CSiec104::Txd_BurstBI(void) //发送变化遥信
{
	WORD		Num,BITxdNum;
	WORD 		wBIEReadPtr;
	TBurstBI 	event;
...
BOOL CSiec104::SearchClass1(void) //检查有无1级数据
{
	// 遥信变位数据
...
BOOL CSiec104::Txd_Soe(void) //发送SOE数据
{
	TEventBI 	event;
	TSysTime	time;
	WORD		BINo;
...
//extern DWORD nTestAIEWriteCnt;
//DWORD nTestAIECnt=0;

BOOL CSiec104::Txd_AIE(void) //发送AIE数据
{
#if(SYSTEM_TYPE == _SYS_KF1300_)
	TEventAI 	event;
	TSysTime	time;
	...
	if(DB_GetAIERead(&nAIEReadPtr) != DB_ERR_OK)
		return FALSE;
		
	for(nAIETxdCnt=0;nAIETxdCnt<GRP_AIENUM&&GetNewAIENum(); nAIETxdCnt++)
	{
		if(::DB_ReadAIE(&event ,(nAIEReadPtr+m_nAIENoAckCnt)%MAX_AIE_NUM) != DB_ERR_OK)
				break;
		
		m_Txd.Buf[ m_Txd.WritePtr++ ] = LOBYTE(event.nNo+AI_ADDR_START); //信息体地址Lo
		m_Txd.Buf[ m_Txd.WritePtr++ ] = HIBYTE(event.nNo+AI_ADDR_START); //信息体地址Hi
		m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //add by xyb
		ConvertRMFix2SecondFloat(event.nNo, event.lValue, &fValue);
		...
		m_Txd.Buf[ m_Txd.WritePtr++ ] = event.bStatus; 

		AbsTimeTo(&event.stTime, &time);
		MSecond = time.Second*1000+time.MSecond;
		m_Txd.Buf[ m_Txd.WritePtr++ ] = LOBYTE(MSecond);  //MSEC L
		m_Txd.Buf[ m_Txd.WritePtr++ ] = HIBYTE(MSecond);  //MSEC H
		m_Txd.Buf[ m_Txd.WritePtr++ ] = time.Minute;	//MSEC H
		m_Txd.Buf[ m_Txd.WritePtr++ ] = time.Hour;  //MSEC H
		m_Txd.Buf[ m_Txd.WritePtr++ ] = time.Day;  //MSEC H
		m_Txd.Buf[ m_Txd.WritePtr++ ] = time.Month;  //MSEC H
		m_Txd.Buf[ m_Txd.WritePtr++ ] = time.Year%100;  //MSEC H

		// 记录个数
		m_nAIENoAckCnt++;
		// 记录发送序号
		m_nAIENewTxSeq = m_dwTxdCount;

//		nTestAIECnt++;
		
#ifdef S104_DEBUG			 
		ReportMsg("S104:	Send AIE enent! No=%d Status=%d",event.nNo,event.bStatus);
#endif		
	}
	...
	for(nAIETxdCnt=0;nAIETxdCnt<GRP_AIENUM&&GetNewAIENum(); nAIETxdCnt++)
	{
		...
		m_Txd.Buf[ m_Txd.WritePtr++ ] = LOBYTE(event.lValue); 
		m_Txd.Buf[ m_Txd.WritePtr++ ] = HIBYTE(event.lValue); 
		m_Txd.Buf[ m_Txd.WritePtr++ ] = event.bStatus; 
...
BOOL CSiec104::Txd_ChangeAI(void) //发送变化遥测数据
{
#if(SYSTEM_TYPE == _SYS_KF1300_)	
	WORD AITxdNum,AINo;
	long 	lValue;
...
BOOL CSiec104::Txd_Class1(void) //发送1级数据
{
	// 遥信变位数据
	if(GetFlag(SF_HAVEBIE))
	{
...
BOOL CSiec104::Txd_Class2(void) //发送2级数据
{
    ...
    if(GetFlag(SF_HAVEAIE))
    {
		if(Txd_AIE())	
			return TRUE;
		else
			SetFlag(SF_HAVEAIE,FLAG_OFF);
    }
...
BOOL CSiec104::Txd68_65_5(void)//召唤某组电度
{
	WORD 	GroupNo;
	PBYTE 	pData=&pRxdFm->Data;
...
BOOL CSiec104::Txd68_65_7(void)//召唤所有电度的确认
{
	Txd_Head(0x65,7);
...
BOOL CSiec104::Txd_AllCIStop(void)//召唤所有电度的确认
{
	Txd_Head(0x65,10);
	m_Txd.Buf[ m_Txd.WritePtr++ ] = 00; //信息体地址Lo
	m_Txd.Buf[ m_Txd.WritePtr++ ] = 00; //信息体地址Hi
...
BOOL CSiec104::Txd_CIGroup(WORD GroupNo,BYTE Reason)//发送一组电度
{
...
BOOL CSiec104::Txd68_67(void)//对钟的确认
{
...
BOOL CSiec104::Txd_AIGroup(WORD GroupNo,BYTE Reason)//发送一组遥测  OK
{
#if(SYSTEM_TYPE == _SYS_KF1300_)
...
BOOL CSiec104::Txd_BIGroup(WORD GroupNo,BYTE Reason)//发送一组遥信
{
	WORD 	BINo,BITxdNum;
	BYTE	BIStatus;
...
BOOL CSiec104::Txd_AllStop(void)//发送总召唤结束帧
{
...
BOOL CSiec104::Txd68_64_14_6(void)//处理总召唤激活命令
{
...
// 函数功能: 发送7个字节的长时标
//=======================================================================================
BOOL CSiec104::Txd68_LongTime(TSysTime *pTime)
{
	WORD MSecond;
...
// 函数功能: 处理报文的头部
//=======================================================================================
BOOL CSiec104::Txd_Head(BYTE Style,WORD Reason)
{
	WORD t_txcount;
	WORD t_rxcount;
...
// 函数功能: 处理报文的尾部
//=======================================================================================
BOOL CSiec104::Txd_Tail(BYTE Num)
{
	BYTE* pBuf;
...
BOOL CSiec104::Txd68_66_AI(WORD AINo)
{
#if(SYSTEM_TYPE == _SYS_KF1300_)
	float	fValue;
	BYTE	*pValue;
...
BOOL CSiec104::Txd68_66_BI(WORD BINo)
{
	BYTE BIStatus;
...
BOOL CSiec104::Txd68_66_CI(WORD CINo)
{
	DWORD CIValue;
...
BOOL CSiec104::Txd68_2E2F(BYTE Reason) //发送遥控预置/执行	
{
	BYTE Style;
	BYTE Num=1;
...
#ifdef S104_DEBUG			 
	LogError("Txd68_2E2F",FILE_LINE,"RetResult =%d!! Command = %d  No= %d  Attrib=%d Reason=%d\r\n\r\n ",m_BO.RetResult,m_BO.Command,m_BO.No,m_BO.Attrib,Reason);
#endif

	return TRUE;
}
/*
BOOL CSiec104::Txd68_3B3C(BYTE Reason) //发送带时标遥控预置/执行	
{
	BYTE Style;
	BYTE Num=1;
	...
	if(m_BO.RetResult==PBO_SUCESS)
	{
		m_Txd.Buf[ m_Txd.WritePtr++ ] = m_BO.Info[1]; //信息体地址Lo
		m_Txd.Buf[ m_Txd.WritePtr++ ] = m_BO.Info[2]; //信息体地址Hi
		m_Txd.Buf[ m_Txd.WritePtr++ ] = m_BO.Info[3]; //信息体地址
		m_Txd.Buf[ m_Txd.WritePtr++ ] = m_BO.Info[4]; //DCO
	}
	else
	{
		m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //信息体地址Lo
		m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //信息体地址Hi
		m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //信息体地址
		m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //DCO
	}

	Txd68_LongTime(); //加7个字节长时标

	Txd_Tail(Num);  
	return TRUE;
}
*/
BOOL CSiec104::Txd68_69_Ack(void)
{
...
BOOL CSiec104::Txd_InitOver(void)
{
...
//	注： 20110418 加
//=========================================================================
BOOL CSiec104::Txd_RelayEvent(void)
{	//诚骋成北塘电厂专用	104规约上传故障电量部分程序
	TAbsTime		AbsTime;
	TSysTime		SysTime;
	WORD			MSecond;  
	BYTE			Num=0;
...
//	if((m_Txd.WritePtr + 3 + 4 + 1 + 4 + 7 + 1 + (2+ 4) * m_pNextRelayEvent->FD_Num) > MAX_TXDFRAME_SIZE)
//		return FALSE;

	// 填充保护动作
	Txd_ElementAddr(DB_GetAddress());
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0;

	m_Txd.Buf[ m_Txd.WritePtr++ ] = LOBYTE(m_pNextRelayEvent->nINF); //INF
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = HIBYTE(m_pNextRelayEvent->nINF); //INF
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //INF
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0; //INF
...
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0x20;
//	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0x35;
	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0;
	m_Txd.Buf[ m_Txd.WritePtr++ ] = 0;
...
//	m_Txd.Buf[m_Txd.WritePtr++] = SysTime.Year-2000;
	...
	// 填充索引号和故障电量
	memcpy(m_Txd.Buf+m_Txd.WritePtr,m_pNextRelayEvent+1,FD_Num*6);
...
// 其他：	 103 typ = 10
//=======================================================================================
BOOL CSiec104::Txd68_146_STSReadEcho(void)
{
...
// 其他：	 103 typ = 10
//=======================================================================================
BOOL CSiec104::Txd68_148_STSSelectEcho(void)
{
	PBYTE 		pData;
	TGenItem	*pelemData;
	TGenData	*pGenData;
...
// 函数改写：
//=======================================================================================
BOOL CSiec104::Txd68_148_STSExecEcho(void)
{//修改（定值区号）执行/撤销返回
	PBYTE 		pData=&pTxdFm->Data;
	TGenData	*pGenData = (TGenData *)&pData[5];
	BYTE 		bINF = *(&pRxdFm->Data+NBYTES_ELMENTADDR);
	STATUS		rc;
...
// 函数改写：
//=======================================================================================
BOOL CSiec104::Txd68_146_STReadEcho(void)
{
	PBYTE 	pData=&pTxdFm->Data;
	TNgd	*Ngd = (TNgd *)&pData[5];
	BOOL  	bisLastFrm = FALSE;
	BYTE 	Num=(MAX_TXDFRAME_SIZE - (sizeof(TS104TxdFm)-1+6)) / (sizeof(TGenItem)-1+4);        //可变结构限定词
	...
	{
		PBYTE pdzData	= (PBYTE)&pData[5] + sizeof(TNgd);			//定值元素开始地址
		PBYTE ptmp = pdzData;
		TGenItem* pDZe;
...
// 其他：	 103 typ = 10
//=======================================================================================
BOOL CSiec104::Txd68_148_STSelectEcho(void )
{
	PBYTE 		pData=&pTxdFm->Data;
	TGenData	*pGenData = (TGenData *)&pData[NBYTES_ELMENTADDR + 2];
...
// 其他：	 103 typ = 10
//=======================================================================================
BOOL CSiec104::Txd68_148_STExecEcho(void)
{
	PBYTE 		pData=&pTxdFm->Data;
	TGenData	*pGenData = (TGenData *)&pData[NBYTES_ELMENTADDR + 2];
	BYTE 		bINF = *(&pRxdFm->Data+NBYTES_ELMENTADDR);
	...
	if(rc != DB_ERR_OK)
	{
		pGenData->Ngd.Number = 0;
		Txd_Tail(0);  
		LogError("Txd68_148_STExecEcho",FILE_LINE,"Execute SW error!");
	}
	else
	{
		pGenData->Ngd.Number = 1;
		...
		Txd_Tail(0x81);  
#ifdef S104_DEBUG			 
		LogError("Txd68_148_STExecEcho",FILE_LINE,"Execute SET OK!");
#endif
	}
...
BOOL CSiec104::Txd_126_RDSJ_DIR_AllDir(void)
{
	BYTE	Style = 126;
	BYTE	Reason = 5;		// 突发
	BYTE    Num = 0;
...
BOOL CSiec104::Txd_126_RDSJ_DIR_SubDir()
{
	BYTE	Style = 126;
	BYTE	Reason = 3;		// 突发
	BYTE    Num = 0;
...
BOOL CSiec104::Txd_126_RDSJ_DIR_Auto(void)
{
	BYTE	Style = 126;
	BYTE	Reason = 3;		// 突发
	BYTE    Num = 0;
...
BOOL	CSiec104::Txd_120_RDSJ_F_FR(BOOL bReady)	//文件准备就绪	
{
	BYTE	Style = 120;
	BYTE	Reason= 13;		//文件传输
	WORD	wFileName;
...
BOOL	CSiec104::Txd_121_RDSJ_F_SR()	//节准备就绪
{
	BYTE 	Style = 121;
	BYTE	Reason= 13;		//文件传输
...
BOOL	CSiec104::Txd_125_RDSJ_F_SG(void)//上传扰动数据 类型标识125：F_SG_NA_1 段
{
	DWORD	len;			//段的长度			
	BYTE 	*pData ;		//段的数据		
	BYTE	Style = 125;
...
BOOL	CSiec104::Txd_123_RDSJ_F_LS(void)	//最后的节,最后的段
{
	BYTE bLsq = 0,chs;
	BYTE Style = 123;
	BYTE Reason=13;		//文件传输
...
DWORD CSiec104::GetNewSoeNum(void)
{
	WORD num=0;
...
DWORD CSiec104::GetNewBieNum(void)
{
	WORD num=0;
...
DWORD CSiec104::GetNewAIENum(void)
{
	WORD num=0;
...
void CSiec104::ClearSentEvent(DWORD dwAckCount)
{
	// 对SOE序号的处理
	if(m_nSOENoAckCnt>0)
	{
