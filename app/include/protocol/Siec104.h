...
class CSiec104 : public CProtocol
{
	DECLARE_MESSAGE_MAP()
//标准规约变量
protected:
...
public:
    BOOL Init(void);//初始化
    CSiec104(DWORD *pdwApp):CProtocol(pdwApp)
	{ 
...
//发送报文处理函数
	...
	BOOL	Rxd68_2E2F(void);//遥控处理
    BOOL	Rxd68_2E2F_6(void);//遥控预置/执行命令
    BOOL	Rxd68_2E2F_8(void);//遥控撤销命令
    BOOL	Txd68_2E2F(BYTE Reason); //发送遥控预置/执行/撤销返校	
	...
	BOOL	Rxd68_147_STSelect();   //整区修改定值预置

	BOOL	Txd68_146_STReadEcho(void);		//取定值返回
	BOOL	Txd68_148_STSelectEcho(void);	//整区修改定值预置返回
	BOOL	Txd68_148_STExecEcho(void);		//修改定值执行/撤销返回
	
	BOOL	Txd68_146_STSReadEcho(void);	//取定值区号返回
	BOOL	Txd68_148_STSExecEcho(void);	//修改定值区号执行/撤销返回
	BOOL	Txd68_148_STSSelectEcho(void);	//修改定值区号预置返回
